...
mConnect Viewer and its medical imaging data. This post covers one of many potential
...
- All static resources for mConnect Viewer are served up by `nginx` when a matching
...
The mConnect Viewer's configuration is imported from a static `.js` file. The
...
| ----------------- | --------------------------------- | ------------------------------------------- |
| mConnect Viewer       | [dockerfile][dockerfile]          | You're reading them now!                    |
